...
위성 이미지 생성을 요청합니다.
동적으로 생성되는 지도를 크롤링 해와야 하기 때문에 별도의 서버에 작업을 요청하고, 크롤링이 완료되면 이미지를 발급할 수 있는 키 값이 제공됩니다.
...
서버간 통신을 위한 api이며, 이미지 생성 요청 시 같이 전달되는 토큰을 통해서만 접근할 수 있습니다.
인증용 토큰이 헤더에 존재하지 않으면 요청은 거부됩니다.
