...
class ImageBuildControllerTest extends SlackMockExtension {
    ...
    void 이미지_제작_요청_테스트() throws Exception {
        List<MapBuildResponse> responses = new ArrayList<>();

        for (int i = 0; i < 3000; i += 1000) {
            for (int j = 0; j < 3000; j += 1000) {
                MapBuildResponse response = MapBuildResponse.builder()
        ...
        List<MapBuildResponse> lst = mapper.readValue(result.getResponse().getContentAsString(),
                new TypeReference<List<MapBuildResponse>>() {
                });
